...
public class UserControllerTest extends BaseControllerTest {
    ...
    void postUser(User user, int status) throws Exception {
        setUser(user, status, "POST");
    }

    void putUser(User user, int status) throws Exception {
        setUser(user, status, "PUT");
    }

    void setUser(User user, int status, String method) throws Exception {
    ...
    User getUser(long id, int status) throws Exception {
        MvcResult result = mockMvc.perform(makeGetRequest("/" + id))
    ...
    void shouldGetUser() throws Exception {
        User user1 = UserTest.getNormalUser(expectedId);
        postUser(user1, 200);
        User user2 = getUser(user1.getId(), 200);
        assertEquals(user1, user2);
    }

    @Test
    void shouldNotGetUser() throws Exception {
        getUser(expectedId, 404);
    }
    ...
    void shouldNotAddToFriendsBecauseWrongId() throws Exception {
        User user = UserTest.getNormalUser(expectedId);
        postUser(user, 200);
        mockMvc.perform(makePutRequest("/" + expectedId + "/friends/" + user.getId(), null))
    ...
    void shouldNotAddToFriendsBecauseWrongFriendId() throws Exception {
        User user = UserTest.getNormalUser(expectedId);
        postUser(user, 200);
        mockMvc.perform(makePutRequest("/" + user.getId() + "/friends/" + expectedId, null))
                .andExpect(status().is(404));
    }
    ...
    void shouldNotDeleteFromFriendsBecauseWrongId() throws Exception {
        shouldAddToFriends();
        mockMvc.perform(makeDeleteRequest("/" + expectedId + "/friends/" + (expectedId - 1)))
                .andExpect(status().is(404));
    }
    ...
    void shouldNotDeleteFromFriendsBecauseWrongFriendId() throws Exception {
        shouldAddToFriends();
        mockMvc.perform(makeDeleteRequest("/" + (expectedId - 2) + "/friends/" + expectedId))
                .andExpect(status().is(404));
    }
    ...
    void shouldNotGetFriends() throws Exception {
        shouldPostNewUser();
        mockMvc.perform(makeGetRequest("/" + expectedId + "/friends"))
                .andExpect(status().is(404));
    }
    ...
    void shouldGetCommonFriends() throws Exception {
        ...
        // make wrong request
        mockMvc.perform(makeGetRequest("/" + expectedId + "/friends/common/" + user2.getId()))
                .andExpect(status().is(404));
        mockMvc.perform(makeGetRequest("/" + user1.getId() + "/friends/common/" + expectedId))
                .andExpect(status().is(404));
